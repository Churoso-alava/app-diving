...
def resumen_lesiones_equipo(df_lesiones: pd.DataFrame) -> dict:
    """
    Estadísticas agregadas: activas actuales vs histórico general.
    """
    if df_lesiones.empty:
        return {
            "total_activas": 0,
            "por_zona": {},
            "total_atletas_afectados": 0,
        }
        
    # Solo activos para el banner de "actuales"
    activas = df_lesiones[df_lesiones["estado"].isin(["Activa", "Recuperación"])]
    
    # Histórico para métricas de equipo
    return {
        "total_activas":      len(activas),
        # por_zona basada en el histórico de todos los eventos
        "por_zona":           df_lesiones["zona_cuerpo"].value_counts().to_dict(),
        # total único de atletas que han tenido algún evento médico
        "total_atletas_afectados": len(df_lesiones["atleta"].unique()),
    }
