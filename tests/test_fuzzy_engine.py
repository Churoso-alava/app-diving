...
def test_atleta_critico_no_mejora_con_plan_recuperacion(motor):
    """
    Atleta con VMP muy baja y VMP_Ratio excesivo NO debe
    ser ÓPTIMO solo porque la carga planificada es de recuperación.
    """
    m = _metricas_base(vmp_hoy=0.45, vmp_ratio=1.65, acwr_carga=1.80, delta_pct=30.0,
...
def test_vmp_ratio_bajo_delta_alarma_no_es_critico_con_vmp_funcional(motor):
    """
    VMP_Ratio bajo + delta alarma + vmp funcional NO debe → CRÍTICO.
    La combinación es compatible con desentrenamiento, no sobrecarga.
    """
    m = _metricas_base(vmp_hoy=1.05, vmp_ratio=0.60, delta_pct=25.0,
    ...
    assert res["indice_fatiga"] >= 25, (
        f"Ratio bajo + delta alarma + VMP funcional → índice={res['indice_fatiga']} "
        f"({res['estado']}). No debería ser CRÍTICO (<25)."
    )

def test_vmp_muy_baja_con_wellness_deficiente_es_critico(motor):
    """VMP muy baja + wellness deficiente → CRÍTICO aunque los ratios sean óptimos."""
    m = _metricas_base(vmp_hoy=0.50, vmp_ratio=1.00, acwr_carga=1.00, delta_pct=5.0, z_meso=-0.5)
...
def test_vmp_alta_con_condiciones_optimas_es_optimo(motor):
    """VMP alta + Ratios óptimos + wellness óptimo → ÓPTIMO."""
    m = _metricas_base(vmp_hoy=2.10, vmp_ratio=1.00, acwr_carga=1.00, delta_pct=-5.0, z_meso=1.5,
...
def test_motor_no_cae_en_fallback_50(motor):
    ...
    """
    m = _metricas_base(vmp_hoy=1.40, vmp_ratio=1.10, acwr_carga=1.10, carga_subjetiva=3.0) 
