...
def insertar_lesion(
    atleta: str, 
    fecha_inicio: date, 
    tipo_evento: str, # 'Lesión' o 'Enfermedad'
    estado: str = "Activa", 
    notas: str = "", 
    # Campos opcionales (mapeo dinámico)
    zona_cuerpo: Optional[str] = None,
    sistema: Optional[str] = None,
    tipo_enfermedad: Optional[str] = None,
    es_contagiosa: bool = False,
    # Hitos temporales
    ...
    try:
        client.table("lesiones").insert(insert_data).execute()
        return True, "Evento médico registrado correctamente."
    except Exception as exc:
...
def cargar_lesiones_activas(atleta: Optional[str] = None) -> pd.DataFrame:
    ...
    try:
        ...
        resp = query.order("fecha_inicio", desc=True).execute()
...
def cargar_historial_lesiones(atleta: str) -> pd.DataFrame:
    ...
    if not client: return pd.DataFrame()
    try:
        resp = client.table("lesiones").select("*").eq("atleta", atleta).order("fecha_inicio", desc=True).execute()
        return pd.DataFrame(resp.data or [])
    except Exception as exc:
        log.error("cargar_historial_lesiones(%s): %s", atleta, exc)
        return pd.DataFrame()

def actualizar_estado_lesion(lesion_id: str, nuevo_estado: str, notas: str = "", fecha_alta: Optional[date] = None) -> Tuple[bool, str]:
    ...
    resp = client.table("lesiones").select("estado").eq("id", lesion_id).execute()
    estado_anterior = resp.data[0]["estado"] if resp.data else "Desconocido"
    ...
    try:
        ...
        client.table("lesiones_historial").insert({
            "lesion_id": lesion_id,
        ...
        return True, f"Estado actualizado a {nuevo_estado} y registrado en historial."
    except Exception as exc:
